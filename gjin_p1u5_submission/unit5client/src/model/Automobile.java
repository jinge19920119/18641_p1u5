...
public class Automobile implements Serializable{
	...
	public String[] getOptionName(int i){
		String[] names= new String[opset.get(i).getOpt().size()];
		for(int j=0;j<names.length;j++){
			names[j]= opset.get(i).getOpt().get(j).getName();
		}
		return names;
	}
	public void printChoice(){
		int num= this.getOptionSetNum();
		for(int i=0;i<num;i++){
			System.out.print(opset.get(i).getName()+" : "+opset.get(i).getChoice().getName());
			System.out.print(", addtional price: "+opset.get(i).getChoice().getPrice());
			System.out.println();
		}
	}
	...
	public void setChoice(int i,String option){
		opset.get(i).setChoice(option);
	}
	...
	/*
	 * read from a properties object and build auto
	 */
